...
class GreedyAlgorithm(FullGurobi):
    ...
    def get_data(self, time):
        ...
        al_dict = {"K":K,"N":N,"C_S":C_S,"C_C":C_C,"A":A,"L":L,"E_kil":E_kil,"S_l":S_l,"u":u,"w":w,"D":D,"b_cloud":b_cloud,"Source":Source}
        self.deployment = simple_deployment_main(theta = 0.5,para_dict = al_dict)

    def solve(self):
        action_result = self.deployment.get_deployment()
        action = self.extract_action(action_result)
        actions = {"migrate":action}
        return actions

    def extract_action(self, deployment):
        """从得出的求解动作中把需要改变的微服务的信息提取出来"""
        K, K_id_list, Device_id_list = self._FullGurobi__matrix_K(self.time)
    ...
    def first_deploy(self):
        ...
        al_dict = {"K":K,"N":N,"C_S":C_S,"C_C":C_C,"A":A,"L":L,"E_kil":E_kil,"S_l":S_l,"u":u,"w":w,"D":D,"b_cloud":b_cloud,"Source":Source}
        self.deployment = simple_deployment_main(theta = 0.5,para_dict = al_dict)
        deployment = self.deployment.get_deployment()

        K, K_id_list, Device_id_list = self._FullGurobi__matrix_K(0)
        action = {}
        for k in range(K):
            device_id = Device_id_list[k]
            application_id = K_id_list[k]
