...
class GurobiAlgorithm:
    ...
    def calculate_Matrix_M_m_k_i(self, device_id:int, application_id:int, microservice_id:int, epsilon:float = 0.01):
        ...
        """
        C_1 = self.calculate_C_m_k_i_1(device_id, application_id, microservice_id)
        C_2 = self.calculate_C_m_k_i_2(device_id, application_id, microservice_id)
    ...
    def find_variable_deploy(self, variable_x, m, k, microservice_id, server_id):
        """
        ...
        return variable_x[microservice_id_index * self.server_num + server_id - 1]
    ...
    def extract_action(self):
        ...
        """
        action = {}
        if self.x == None or self.m == None or self.k == None:
            raise ValueError("There are problem in solving!")
        x = self.x
        microservice_ids = self.application_contain_microservice_ids(self.k)
        microservice_number = len(microservice_ids)
        for idx in range(microservice_number):
            if idx == 0:continue
            candidate_list = [round(x[i + idx*self.server_num].x) for i in range(self.server_num)]
            server_id = candidate_list.index(1) + 1
            if self.microservice_deployment_last[self.m][self.k][microservice_ids[idx]] != server_id:
                action = self.add_migrate(action, self.m, self.k, microservice_ids[idx], server_id)
    ...
    def get_data(self, time:int):
        ...
        model,x = self.gurobi_create(self.server_num * (self.application_contain_microservice_number(k) + 1))
        self.constraint_1(model, x, m, k, time, epsilon=self.epsilon)
        self.constraint_2(model, x, m, k, time, epsilon=self.epsilon)
        self.constraint_3(model, x, m, k, time, epsilon=self.epsilon)
        self.constraint_4(model, x, m, k, time, epsilon=self.epsilon)
        self.Objective_function(model, x, m, k, epsilon=self.epsilon)
